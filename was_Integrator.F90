...
! --- --- ---
       ...
       END IF ! if istage == 1 elseif ros_NewF(istage)
       !slim: CALL WCOPY(N,Fcn,1,K(ioffset+1),1)
       K(ioffset+1:ioffset+N) = Fcn(1:N)!SPC_MAP(1:rNVAR))
       DO j = 1, istage-1
         HC = ros_C((istage-1)*(istage-2)/2+j)/(Direction*H)
!         CALL WAXPY(NVAR,HC,K(rNVAR*(j-1)+1),1,K(ioffset+1),1)
         CALL WAXPY(N,HC,K(N*(j-1)+1),1,K(ioffset+1),1)
      END DO
      IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
         HG = Direction*H*ros_Gamma(istage)
...
!~~~>    Compute LU decomposition
     CALL ros_cDecomp( Ghimj(JVS_MAP(1:cNONZERO)), Pivot, ISING )
     IF (ISING == 0) THEN
!~~~>    If successful done
        Singular = .FALSE.
     ELSE ! ISING .ne. 0
